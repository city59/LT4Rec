...
class Aggregator(nn.Module):
    """
    Local Weighted Intention aggregation scheme
    """
    ...
    def forward(self, entity_emb, user_emb,
        ...
        """LWS for user representation learning on user-item"""
        u = None
        for clus_iter in range(self.n_iter):
            ...
            if clus_iter < self.n_iter - 1:
                ...
                u = squash.unsqueeze(1) * F.normalize(u, dim=1)
            u += user_emb
        user_agg = u       # user embedding after aggregate
...
class Recommender(nn.Module):
    #   data_config为数据配置信息，args_config为参数配置信息，graph为KG，adj_mat图的邻接矩阵，作为GCN输入
    ...
    def forward(self, cf_batch):
        ...
        u_e_1 = user_gcn_emb[user]    # user embedding after aggregate
        i_e_1 = entity_gcn_emb[item]      # item embedding after aggregate

        '''build u-i interact graph'''
        ...
        u_e_2 = user_lightgcn_emb[user]     # u-i user embedding
        i_e_2 = item_lightgcn_emb[item]     # u-i item embedding
        ...
        loss_contrast = loss_contrast + self.calculate_loss_1(item_1, i_e_2)  # Global item contrastive
        loss_contrast = loss_contrast + self.calculate_loss_2(user_1, u_e_2)  # Global user contrastive
    ...
    def calculate_loss(self, A_embedding, B_embedding):
        self.tau = nn.Parameter(torch.tensor(0.6))  # 温度参数
    ...
    def create_bpr_loss(self, users, items, labels, loss_contrast):
        ...
        return mar_loss + emb_loss + 0.001*loss_contrast, scores, mar_loss, emb_loss
