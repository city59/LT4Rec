...
from modules.LT4Rec import Recommender
...
if __name__ == '__main__':
    ...
    """build dataset"""
    train_cf, test_cf, user_dict, n_params, graph, triplets, relation_dict, mat_list = load_data(args)     # 训练集、测试集、用户字典、参数数量、图数据、三元组数据、关系字典和矩阵列表
    adj_mat_list, norm_mat_list, mean_mat_list = mat_list       # 邻接矩阵、归一化矩阵和平均矩阵
    ...
    for epoch in range(args.epoch):
        ...
        if epoch % 10 == 9 or epoch == 1:
            ...
            """save weight"""
            if ret['recall'][4] == cur_best_pre_0 and args.save:
                print("Saving model...")
